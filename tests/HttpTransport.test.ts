...
// On Windows + Node 24+ on GitHub Actions, the after() hook that closes
// the http.Server triggers a libuv abort:
//   "Assertion failed: !(handle->flags & UV_HANDLE_CLOSING),
//    file src\\win\\async.c, line 76"
// The auth logic itself works (the 4 cases pass; abort is in cleanup) and
// the suite runs cleanly on macOS, Linux, and Windows + Node 22 — so we
// skip on that one combo rather than masking real failures elsewhere.
const NODE_MAJOR = Number.parseInt(process.versions.node.split('.')[0], 10);
const SKIP_HTTP_SERVER_TESTS =
  process.platform === 'win32' && NODE_MAJOR >= 24
    ? 'Known Node 24+ libuv abort on Windows during http.Server close.'
    : false;

describe(
  'startHttpTransport bearer auth',
  {skip: SKIP_HTTP_SERVER_TESTS},
  () => {
