...
describe('comments', () => {
  it('formats empty comments in McpResponse', async () => {
    await withMcpContext(async (response, context) => {
      ...
      if ('comments' in structuredContent) {
        assert.deepStrictEqual(structuredContent.comments, []);
      } else {
        assert.fail('Expected comments in structuredContent');
      }
    });
  });

  it('formats comments with resolved targets in McpResponse', async () => {
    await withMcpContext(async (response, context) => {
      const page = context.getSelectedMcpPage();
      sinon.stub(page, 'resolveBackendNodeId').resolves('element-uid-42');
      sinon.stub(page, 'resolveCdpRequestId').returns(7);
      ...
      assert.strictEqual(getTextContent(content[0]), expectedText);
      if ('comments' in structuredContent) {
        assert.deepStrictEqual(structuredContent.comments, [
          {
            id: 'comment-1',
