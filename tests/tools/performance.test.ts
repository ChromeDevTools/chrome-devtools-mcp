...
function getBrowserlessContext(): Promise<Context> {
  const page = {
    isClosed: () => false,
    url: () => '',
    on: () => {},
  ...
  const browser = {pages: async () => [page]} as unknown as import('puppeteer').Browser;
  return McpContext.from(browser, logger('test'), {experimentalDevToolsDebugging: false}, Locator);
}
