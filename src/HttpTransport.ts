...
export async function startHttpTransport(
  ...
  return {
    ...
    async close() {
      ...
      sessions.clear();
      // Force-close keep-alive connections so httpServer.close() actually
      // completes promptly. Without this, on Windows libuv can abort with
      // UV_HANDLE_CLOSING when the runtime tears down a server that still
      // has idle (but not yet GC'd) keep-alive sockets attached.
