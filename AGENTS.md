...
- **Mocking Puppeteer `Page`**:
  - Puppeteer's `Page` is an abstract class, so stub the concrete `CdpPage` subclass re-exported as a value from `src/third_party/index.ts`.
  - Return `sinon.SinonStubbedInstance<Page>` from `createMockPuppeteerPage()`.
  - Common internal dependencies required by constructors (such as `_client` for `ConsoleCollector`) should be configured inside `createMockPuppeteerPage()` in `tests/mocks.ts` rather than in individual tests.
